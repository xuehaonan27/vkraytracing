#ifndef BASE_BUFFER_UTIL_HPP
#define BASE_BUFFER_UTIL_HPP
...
void BufferUtil::CopyFromStagingBuffer(
    ...
    auto stagingBuffer =
        std::make_unique<Buffer>(device, contentSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
...
void BufferUtil::CreateDeviceBuffer(
    ...
    memory.reset(
        new DeviceMemory(buffer->AllocateMemory(allocateFlags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
    );
