...
class Application {
  public:
    ...
    bool HasSwapChain() const {
        return swapChain_.operator bool();
    }
  ...
  protected:
    ...
    const std::vector<Base::UniformBuffer>& UniformBuffers() const {
        return uniformBuffers_;
    }

    const class GraphicsPipeline& GraphicsPipeline() const {
        return *graphicsPipeline_;
    }

    const class FrameBuffer& SwapChainFrameBuffer(const size_t i) const {
        return swapChainFramebuffers_[i];
    }

    virtual const Base::Scene& GetScene() const = 0;
    virtual Base::UniformBufferObject GetUniformBufferObject(VkExtent2D extent) const = 0;
  ...
  private:
    ...
    std::vector<Base::UniformBuffer> uniformBuffers_;
