...
TextureImage::TextureImage(Base::CommandPool& commandPool, const RayTrace::Texture& texture) {
    ...
    auto stagingBuffer =
        std::make_unique<Base::Buffer>(device, imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
    ...
    imageMemory_.reset(
        new Base::DeviceMemory(image_->AllocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
    );
    imageView_.reset(
        new Base::ImageView(device, image_->Handle(), image_->Format(), VK_IMAGE_ASPECT_COLOR_BIT)
    );
    sampler_.reset(new Base::Sampler(device, Base::SamplerConfig()));
