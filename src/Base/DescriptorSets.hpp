#ifndef BASE_DESCRIPTOR_SETS_HPP
#define BASE_DESCRIPTOR_SETS_HPP
...
class DescriptorSets final {
  public:
    ...
    VkDescriptorSet Handle(size_t index) const {
        return descriptorSets_[index];
    }

    VkWriteDescriptorSet Bind(
        size_t index,
        uint32_t binding,
    ...
    VkWriteDescriptorSet Bind(
        size_t index,
        uint32_t binding,
        const VkWriteDescriptorSetAccelerationStructureKHR& structureInfo,
        uint32_t count = 1
    ) const;
...
#endif // BASE_DESCRIPTOR_SETS_HPP
