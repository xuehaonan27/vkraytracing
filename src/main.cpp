...
void PrintVulkanInstanceInformation(const Base::Application& application, bool benchmark);
void PrintVulkanLayersInformation(const Base::Application& application, bool benchmark);
void PrintVulkanDevices(
    const Base::Application& application,
    const std::vector<uint32_t>& visible_devices
);
void PrintVulkanSwapChainInformation(const Base::Application& application, bool benchmark);
void SetVulkanDevice(Base::Application& application, const std::vector<uint32_t>& visible_devices);
} // namespace
...
int main(int argc, const char* argv[]) noexcept {
    try {
        ...
        RayTracer application(
            userSettings,
            windowConfig,
            static_cast<VkPresentModeKHR>(options.PresentMode)
        );
        ...
        Console::Write(Severity::Fatal, [&exception]() {
            std::cerr << "FATAL: " << exception.what() << std::endl;
        });
    }

    catch (...) {
        Console::Write(Severity::Fatal, []() {
...
void PrintVulkanInstanceInformation(const Base::Application& application, const bool benchmark) {
    if (benchmark) {
        return;
    }
    ...
    for (const auto& extension : application.Extensions()) {
        std::cout << "- " << extension.extensionName << " (" << Base::Version(extension.specVersion)
                  << ")" << std::endl;
    }
...
void PrintVulkanLayersInformation(const Base::Application& application, const bool benchmark) {
    if (benchmark) {
        return;
    }
    ...
    for (const auto& layer : application.Layers()) {
        std::cout << "- " << layer.layerName << " (" << Base::Version(layer.specVersion) << ")"
...
void PrintVulkanDevices(
    ...
    for (const auto& device : application.PhysicalDevices()) {
        ...
        if (!visible_devices.empty()
            && std::find(visible_devices.begin(), visible_devices.end(), prop.deviceID)
                == visible_devices.end()) {
            break;
        }

        const Base::Version vulkanVersion(prop.apiVersion);
        const Base::Version driverVersion(prop.driverVersion, prop.vendorID);

        std::cout << "- [" << prop.deviceID << "] ";
        std::cout << Base::Strings::VendorId(prop.vendorID) << " '" << prop.deviceName;
        std::cout << "' (";
        std::cout << Base::Strings::DeviceType(prop.deviceType) << ": ";
        std::cout << "vulkan " << vulkanVersion << ", ";
        std::cout << "driver " << driverProp.driverName << " " << driverProp.driverInfo << " - "
                  << driverVersion;
        std::cout << ")" << std::endl;
    }
...
void PrintVulkanSwapChainInformation(const Base::Application& application, const bool benchmark) {
...
void SetVulkanDevice(Base::Application& application, const std::vector<uint32_t>& visible_devices) {
    ...
    const auto result = std::find_if(
        ...
        [&](const VkPhysicalDevice& device) {
            ...
            const auto extensions = Base::GetEnumerateVector(
                device,
            ...
            const auto queueFamilies =
                Base::GetEnumerateVector(device, vkGetPhysicalDeviceQueueFamilyProperties);
