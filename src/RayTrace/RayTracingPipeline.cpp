...
RayTracingPipeline::RayTracingPipeline(
    ...
    const std::vector<Base::DescriptorBinding> descriptorBindings = {
    ...
    descriptorSetManager_.reset(
        new Base::DescriptorSetManager(device, descriptorBindings, uniformBuffers.size())
    );
    ...
    pipelineLayout_.reset(
        new Base::PipelineLayout(device, descriptorSetManager_->DescriptorSetLayout())
    );

    // Load shaders.
    const Base::ShaderModule rayGenShader(device, "../assets/shaders/RayTracing.rgen.spv");
    const Base::ShaderModule missShader(device, "../assets/shaders/RayTracing.rmiss.spv");
    const Base::ShaderModule closestHitShader(device, "../assets/shaders/RayTracing.rchit.spv");
    const Base::ShaderModule proceduralClosestHitShader(
        device,
        "../assets/shaders/RayTracing.Procedural.rchit.spv"
    );
    const Base::ShaderModule proceduralIntersectionShader(
        device,
    ...
    Check(
        deviceProcedures.vkCreateRayTracingPipelinesKHR(
            device.Handle(),
            nullptr,
            nullptr,
            1,
