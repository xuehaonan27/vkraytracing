...
namespace {
    ...
    size_t GetEntrySize(
        ...
        return RoundUp(
            rayTracingProperties.ShaderGroupHandleSize() + maxArgs,
...
ShaderBindingTable::ShaderBindingTable(
    ...
    const size_t sbtSize = rayGenPrograms.size() * rayGenEntrySize_
        + missPrograms.size() * missEntrySize_ + hitGroups.size() * hitGroupEntrySize_;

    // Allocate buffer & memory.
    const auto& device = rayTracingProperties.Device();

    buffer_.reset(new Base::Buffer(
        device,
        sbtSize,
        VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT
            | VK_BUFFER_USAGE_SHADER_BINDING_TABLE_BIT_KHR
    ));
    bufferMemory_.reset(new Base::DeviceMemory(buffer_->AllocateMemory(
    ...
    Check(
        deviceProcedures.vkGetRayTracingShaderGroupHandlesKHR(
            device.Handle(),
    ...
    uint8_t* pData = static_cast<uint8_t*>(bufferMemory_->Map(0, sbtSize));

    pData += CopyShaderData(
        pData,
