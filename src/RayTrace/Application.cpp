...
#include "Base/BufferUtil.hpp"
#include "Base/Image.hpp"
#include "Base/ImageMemoryBarrier.hpp"
#include "Base/ImageView.hpp"
#include "Base/PipelineLayout.hpp"
#include "Base/Scene.hpp"
#include "Base/SingleTimeCommands.hpp"
...
namespace {
    ...
    GetTotalRequirements(const std::vector<TAccelerationStructure>& accelerationStructures) {
        ...
        for (const auto& accelerationStructure : accelerationStructures) {
            total.accelerationStructureSize +=
                accelerationStructure.BuildSizes().accelerationStructureSize;
...
Application::Application(
    ...
    Base::Application(windowConfig, presentMode, enableValidationLayers) {}
...
void Application::SetPhysicalDevice(
    ...
    Base::Application::SetPhysicalDevice(
        physicalDevice,
        requiredExtensions,
        deviceFeatures,
        &rayTracingFeatures
...
void Application::OnDeviceSet() {
    Base::Application::OnDeviceSet();

    deviceProcedures_.reset(new Base::DeviceProcedures(Device()));
...
void Application::CreateAccelerationStructures() {
    ...
    Base::SingleTimeCommands::Submit(CommandPool(), [this](VkCommandBuffer commandBuffer) {
...
void Application::CreateSwapChain() {
    Base::Application::CreateSwapChain();
    ...
    const std::vector<ShaderBindingTable::Entry> rayGenPrograms = {
        {rayTracingPipeline_->RayGenShaderIndex(), {}}
    };
    const std::vector<ShaderBindingTable::Entry> missPrograms = {
        {rayTracingPipeline_->MissShaderIndex(), {}}
    };
    ...
    shaderBindingTable_.reset(new ShaderBindingTable(
        *deviceProcedures_,
        *rayTracingPipeline_,
...
void Application::DeleteSwapChain() {
    ...
    Base::Application::DeleteSwapChain();
}
...
void Application::Render(
    ...
    Base::ImageMemoryBarrier::Insert(
        commandBuffer,
        accumulationImage_->Handle(),
        subresourceRange,
        0,
    ...
    vkCmdBindPipeline(
        ...
        rayTracingPipeline_->Handle()
    );
    vkCmdBindDescriptorSets(
        commandBuffer,
        VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
        rayTracingPipeline_->PipelineLayout().Handle(),
        0,
        1,
        descriptorSets,
        0,
...
void Application::CreateBottomLevelStructures(VkCommandBuffer commandBuffer) {
    ...
    for (const auto& model : scene.Models()) {
        ...
        model.Procedural() ? geometries.AddGeometryAabb(scene, aabbOffset, 1, true)
                           : geometries.AddGeometryTriangles(
                                 scene,
                                 vertexOffset,
                                 vertexCount,
                                 indexOffset,
        ...
        vertexOffset += vertexCount * sizeof(RayTrace::Vertex);
    ...
    bottomBuffer_.reset(new Base::Buffer(
        Device(),
        total.accelerationStructureSize,
        VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR
            | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT
    ));
    bottomBufferMemory_.reset(new Base::DeviceMemory(bottomBuffer_->AllocateMemory(
        VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
    )));
    bottomScratchBuffer_.reset(new Base::Buffer(
        Device(),
        total.buildScratchSize,
        VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR
            | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT
    ));
    bottomScratchBufferMemory_.reset(new Base::DeviceMemory(bottomScratchBuffer_->AllocateMemory(
        VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
    )));
    ...
    for (size_t i = 0; i != bottomAs_.size(); ++i) {
        bottomAs_[i].Generate(
            commandBuffer,
            *bottomScratchBuffer_,
            scratchOffset,
            *bottomBuffer_,
            resultOffset
        );
...
void Application::CreateTopLevelStructures(VkCommandBuffer commandBuffer) {
    ...
    Base::BufferUtil::CreateDeviceBuffer(
        CommandPool(),
        "TLAS Instances",
    ...
    topBuffer_.reset(new Base::Buffer(
        Device(),
        total.accelerationStructureSize,
        VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR
    ));
    topBufferMemory_.reset(
        new Base::DeviceMemory(topBuffer_->AllocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
    );

    topScratchBuffer_.reset(new Base::Buffer(
        Device(),
        total.buildScratchSize,
        VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR
            | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT
    ));
    topScratchBufferMemory_.reset(new Base::DeviceMemory(topScratchBuffer_->AllocateMemory(
        VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
    )));
...
void Application::CreateOutputImage() {
    ...
    accumulationImage_.reset(new Base::Image(
        Device(),
        extent,
    ...
    accumulationImageMemory_.reset(new Base::DeviceMemory(
        accumulationImage_->AllocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)
    ));
    accumulationImageView_.reset(new Base::ImageView(
        Device(),
        accumulationImage_->Handle(),
        VK_FORMAT_R32G32B32A32_SFLOAT,
    ...
    outputImage_.reset(new Base::Image(
        Device(),
        extent,
        format,
        tiling,
    ...
    outputImageMemory_.reset(
        new Base::DeviceMemory(outputImage_->AllocateMemory(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
    );
    outputImageView_.reset(
        new Base::ImageView(Device(), outputImage_->Handle(), format, VK_IMAGE_ASPECT_COLOR_BIT)
    );
