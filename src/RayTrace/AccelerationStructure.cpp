...
AccelerationStructure::~AccelerationStructure() {
    if (accelerationStructure_ != nullptr) {
        deviceProcedures_
            .vkDestroyAccelerationStructureKHR(device_.Handle(), accelerationStructure_, nullptr);
        accelerationStructure_ = nullptr;
    }
}

VkAccelerationStructureBuildSizesInfoKHR
AccelerationStructure::GetBuildSizes(const uint32_t* pMaxPrimitiveCounts) const {
    ...
    const uint64_t ScratchAlignment =
        rayTracingProperties_.MinAccelerationStructureScratchOffsetAlignment();

    sizeInfo.accelerationStructureSize =
        RoundUp(sizeInfo.accelerationStructureSize, AccelerationStructureAlignment);
...
void AccelerationStructure::CreateAccelerationStructure(
    ...
    Check(
        deviceProcedures_.vkCreateAccelerationStructureKHR(
            device_.Handle(),
            &createInfo,
            nullptr,
