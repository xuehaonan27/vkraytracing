...
Base::UniformBufferObject RayTracer::GetUniformBufferObject(const VkExtent2D extent) const {
    const auto& init = cameraInitialSate_;

    Base::UniformBufferObject ubo = {};
    ubo.ModelView = modelViewController_.ModelView();
    ubo.Projection = glm::perspective(
        glm::radians(userSettings_.FieldOfView),
...
void RayTracer::CreateSwapChain() {
    ...
    userInterface_.reset(
        new UserInterface(CommandPool(), SwapChain(), DepthBuffer(), userSettings_)
    );
...
void RayTracer::DrawFrame() {
    ...
    if (resetAccumulation_ || userSettings_.RequiresAccumulationReset(previousSettings_)
    ...
    numberOfSamples_ = glm::clamp(
        userSettings_.MaxNumberOfSamples - totalNumberOfSamples_,
        0u,
...
void RayTracer::Render(
    ...
    userSettings_.IsRayTraced
        ? RayTrace::Application::Render(commandBuffer, currentFrame, imageIndex)
        : Base::Application::Render(commandBuffer, currentFrame, imageIndex);
...
void RayTracer::OnCursorPosition(const double xpos, const double ypos) {
    if (!HasSwapChain() || userSettings_.Benchmark || userInterface_->WantsToCaptureKeyboard()
...
void RayTracer::OnMouseButton(const int button, const int action, const int mods) {
    if (!HasSwapChain() || userSettings_.Benchmark || userInterface_->WantsToCaptureMouse()) {
        return;
    }
...
void RayTracer::LoadScene(const uint32_t sceneIndex) {
    ...
    if (textures.empty()) {
        textures.push_back(
            RayTrace::Texture::LoadTexture("../assets/textures/white.png", Base::SamplerConfig())
        );
    }

    scene_.reset(new Base::Scene(CommandPool(), std::move(models), std::move(textures)));
...
void RayTracer::CheckAndUpdateBenchmarkState(double prevTime) {
    ...
    if (periodTotalFrames_ == 0) {
        std::cout << std::endl;
        std::cout << "Benchmark: Start scene #" << sceneIndex_ << " '"
                  << SceneList::AllScenes[sceneIndex_].first << "'" << std::endl;
    ...
    // Print out the frame rate at regular intervals.
    {
        ...
        if (periodTotalFrames_ != 0
            && static_cast<uint64_t>(prevTotalTime / period)
                != static_cast<uint64_t>(totalTime / period)) {
    ...
    {
        const bool timeLimitReached = periodTotalFrames_ != 0
            && Window().GetTime() - sceneInitialTime_ > userSettings_.BenchmarkMaxTime;
        ...
        if (timeLimitReached || sampleLimitReached) {
            if (!userSettings_.BenchmarkNextScenes
                || static_cast<size_t>(userSettings_.SceneIndex)
...
void RayTracer::CheckFramebufferSize() const {
    ...
    if (userSettings_.Benchmark && cfg.Fullscreen
        && (fbSize.width != cfg.Width || fbSize.height != cfg.Height)) {
