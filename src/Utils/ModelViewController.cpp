...
bool ModelViewController::OnKey(
    const int key,
    const int scancode,
    const int action,
    const int mods
...
bool ModelViewController::UpdateCamera(const double speed, const double timeDelta) {
    ...
    const bool updated = cameraMovingLeft_ || cameraMovingRight_ || cameraMovingBackward_
        || cameraMovingForward_ || cameraMovingDown_ || cameraMovingUp_ || cameraRotY_ != 0
        || cameraRotX_ != 0;
