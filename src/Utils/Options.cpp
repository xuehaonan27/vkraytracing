...
Options::Options(const int argc, const char* argv[]) {
    ...
    benchmark
        .add_options()("next-scenes", bool_switch(&BenchmarkNextScenes)->default_value(false), "Load the next scene once the sample or time limit is reached.")(
            "max-time",
    ...
    scene.add_options()(
        "scene",
        value<uint32_t>(&SceneIndex)->default_value(1),
    ...
    vulkan.add_options()(
        "visible-device",
        value<std::vector<uint32_t>>(&VisibleDevices),
        "Explicitly set which Vulkan device ID is visible (can be repeated for multiple devices). If unspecified, all devices are visible."
    );
