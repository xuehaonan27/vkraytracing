...
void CheckVulkanResultCallback(const VkResult err) {
    if (err != VK_SUCCESS) {
        throw std::runtime_error(std::string("ImGui Vulkan error (") + ToString(err) + ")");
    }
}
...
UserInterface::UserInterface(
    ...
    descriptorPool_.reset(new Base::DescriptorPool(device, descriptorBindings, 1));
    renderPass_.reset(new Base::RenderPass(
        swapChain,
        depthBuffer,
    ...
    Base::SingleTimeCommands::Submit(commandPool, [](VkCommandBuffer commandBuffer) {
...
void UserInterface::DrawOverlay(const Statistics& statistics) {
    ...
    if (ImGui::Begin("Statistics", &Settings().ShowOverlay, flags)) {
        ImGui::Text(
            "Statistics (%dx%d):",
            statistics.FramebufferSize.width,
